...
Dans la suite nous allons calculer sur la courbe _prime256v1_. Les paramètres de cette courbe sont donnés ici <<18>>, sous le nom _secp256r1_.

Assez recopié de documents de mathématiques trouvés sur Internet, ces équations nous serviront peu, c'était pour présenter les deux familles.
Nous allons maintenant nous intéresser aux courbes sur _F~p~_.

Ce tutorial est très bien fait <<18>>.
Le document suivant donne les paramètres des courbes{nbsp}<<19>>.
Enfin le document brainpool définit les paramètres de certaines courbes{nbsp}<<20>>.

==== La courbe prime256v1
...
Pour représenter un point de la courbe nous utilisons la convention _point[0] = abcisse du point_ et _point[1] = ordonnée du point_.
...
==== L'arithmétique sur les courbes

Les calculs sont presque tous faits footnote:[Dans la signature ECDSA certains calculs sont faits modulo _n_, l'ordre du groupe associé à la courbe. Dans ce cas nous le préciserons.]
...
Les formules qui en découlent sont les suivantes, pour _R{nbsp}={nbsp}P{nbsp}+{nbsp}Q_ de coordonnées (_r~x~_, _r~y~_), (_p~x~_, _p~y~_) et (_q~x~_, _q~y~_) respectivement, _P_ étant différent de _Q_{nbsp}:
...
.Multiplication par un entier _k_ de grande taille

Les _k_ utilisés en pratique pouvant être très grands (de l'ordre de 2^256^ avec _prime256v1_), il n'est pas possible de calculer _kP_ en ajoutant _P_ à lui-même _k_ fois.

Il faut utiliser l'algorithme d'exponentiation rapide vu précédemment{nbsp}<<9>>.
Remplacer la multiplication d'entiers par l'addition de deux points et l'élévation à la puissance par la multiplication.
