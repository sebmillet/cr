...
Les courbes binaires fonctionnent selon les mêmes principes mais les calculs sont faits sur des polynômes, et si l'on souhaite y mener des calculs avec _bc_ ou _python_, cela nécessite de coder spécifiquement leur arithmétique. Ce n'est pas l'objet de ce document et nous examinerons seulement les courbes sur _F~p~_.

==== L'arithmétique des courbes sur _F~p~_

Les calculs sont presque tous faits footnote:[Dans la signature ECDSA certains calculs sont faits modulo _n_, l'ordre du groupe associé à la courbe. Cela sera signalé. À défaut, tous les calculs sont faits modulo _p_.]
...
Les formules qui en découlent sont les suivantes, pour _R{nbsp}={nbsp}P{nbsp}+{nbsp}Q_ de coordonnées (_r~x~_, _r~y~_), (_p~x~_, _p~y~_) et (_q~x~_, _q~y~_) respectivement, _P_ étant différent de _Q_ et non aligné verticalement (_p~x~_ est différent de _q~x~_){nbsp}:
