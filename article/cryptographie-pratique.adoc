...
* Windows{nbsp}: les binaires sont accessibles à l'URL{nbsp}<<6>>.
...
L'éditeur de texte (natif) adéquat pour ce type de transformation est *_notepad++_*, disponible à cette URL{nbsp}<<7>>.
...
La fonction _powmod_ met en oeuvre l'algorithme d'exponentiation rapide, décrit à cette URL{nbsp}<<8>>.
En fait nous sommes dans un contexte modulaire et d'après Wikipédia le nom exact de l'algorithme est _exponentiation modulaire_. Un article y est consacrée{nbsp}<<9>>.
...
_dder_ est disponible à l'URL{nbsp}<<10>>.

_pkfile_ est disponible à l'URL{nbsp}<<11>>.

.Exécutable openssl

* Sous Linux ce programme est disponible par défaut.
* Sous Windows il faut trouver un binaire à télécharger.
Le plus simple est de télécharger la version GNUWin32, disponible à cette URL{nbsp}<<12>>.
+
...
. À la main
. À l'aide de l'utilitaire _pkfile_. _pkfile_ est disponible à cette URL{nbsp}<<11>>.
