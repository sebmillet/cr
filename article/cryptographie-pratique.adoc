...
Ce certificat aura pour clé la clé contenue dans le fichier _k.pem_, de type _prime256v1_.
...
Les formules sont disponibles ici par exemple{nbsp}<<24>>.
Ce tutoriel (en anglais) est très bien fait et explique de nombreux calculs{nbsp}<<25>>.
Jusqu'à présent _k_ désignait la clé privée, ci-dessous _k_ est un entier choisi aléatoirement. La clé privée est notée _d_.
...
. Calculer _R{nbsp}={nbsp}(R~x~,{nbsp}R~y~){nbsp}={nbsp}kG_ puis _r{nbsp}={nbsp}R~x~ mod n_) et si _r_ est égal à _0_, recommencer

. Calculer _s{nbsp}={nbsp}(H(m)+rd)/k mod n_

Le couple _(r,{nbsp}s)_ est la signature du hash du message _m_, représenté par la fonction _H(m)_.

.Vérification d'une signature ECDSA

. Calculer _u~1~{nbsp}={nbsp}H(m)(s^-1^{nbsp}mod{nbsp}n)_ mod n et _u~2~{nbsp}={nbsp}r(s^-1^{nbsp}mod{nbsp}n)_

. Calculer _P{nbsp}={nbsp}(P~x~,{nbsp}P~y~){nbsp}={nbsp}u~1~G{nbsp}+{nbsp}u~2~Q_ et calculer _p{nbsp}={nbsp}P~x~{nbsp}mod{nbsp}n_

. Vérifier que _p{nbsp}={nbsp}r_

.Vérification de notre certificat auto-signé

Une fois de plus l'encodage est défini par la RFC 5280. Il s'agit de la définition suivante{nbsp}:
...
La valeur de _(r,{nbsp}s)_ est la troisième séquence du certificat que _pkfile_ a noté _1.3.1_.
Récupérer les valeurs de _r_ et _s_, soit avec la commande _pkfile_{nbsp}:
...
soit avec la commande _openssl_ (reprendre le numéro d'octet du dernier _BIT STRING_ dans l'option _strparse_){nbsp}:
...
Copier/coller le résultat dans le fichier sign.b{nbsp}:

Nous avons également besoin de _H(m)_, que nous noterons _h_ dans notre script (dans certains tutoriels il est noté _e_). Comme vu avec le RSA il faut extraire la partie tbsCertificate de notre certificat et en calculer le hash.

Quel hash{nbsp}? Le certificat indique que l'algorithme est _ecdsa-with-SHA256_, donc _h{nbsp}={nbsp}SHA256(tbsCertificate)_.

.Calcul de h

Extraire tbsCertificate dans _tbs.der_ puis en calculer le hash SHA256, avec les commandes suivantes{nbsp}:
...
Au final le fichier _sign.b_ ressemble à ceci{nbsp}:
