...
Rappelons que c'est _S_ (variable _s_ dans _val.b_) qui doit être élevé à la puissance _e_, modulo _n_. Nous écrivons _2{nbsp}*{nbsp}8_ au lieu de _16_ pour être certain de définir _obase_ à _16_ quelle que soit la valeur d'_ibase_.
...
Le point _Q_ est la clé publique, l'entier _k_ est la clé privéefootnote:[Un peu plus loin la clé privée sera notée _s_.].
...
Exécuter footnote:[Enregistrer une clé privée dans un fichier non protégé par mot de passe (chiffré) est une mauvaise pratique. D'autre part le fichier doit être enregistré dans un endroit parfaitement sécurisé, ne jamais être transféré à travers un réseau, etc. Ici cela n'a pas d'importance puisque la clé est une clé de test.]
