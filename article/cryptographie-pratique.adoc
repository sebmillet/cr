...
[NOTE]
On laisse le soin au lecteur de voir pourquoi le deuxième _mod{nbsp}n_ est facultatif dans les formules de calcul de _u~1~_ et _u~2~_.
...
Récupérer les valeurs de _r_ et _s_ du certificat et les écrire dans le fichier _sign.b_.
...
Nous avons également besoin de _H(m)_, que nous noterons _h_ dans notre script (dans certains tutoriels il est noté _e_). _h_ a été défini par _secp256r1.b_ (c'est le cofacteur) mais nous n'en avons pas besoin. Comme vu avec le RSA il faut extraire la partie tbsCertificate de notre certificat et en calculer le hash.

Quel hash{nbsp}? Le certificat indique que l'algorithme est _ecdsa-with-SHA256_, donc _h{nbsp}={nbsp}SHA256(tbsCertificate)_.
...
Après ces manipulations le fichier _sign.b_ contient _r_, _s_ et _h_.
