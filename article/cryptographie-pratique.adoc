...
Les formules qui en découlent sont les suivantes, pour _R{nbsp}={nbsp}P{nbsp}+{nbsp}Q_ de coordonnées (_r~x~_, _r~y~_), (_p~x~_, _p~y~_) et (_q~x~_, _q~y~_) respectivement, _P_ étant différent de Q et non aligné verticalement avec _Q_ (_p~x~_ est différent de _q~x~_){nbsp}:
...
Le calcul de l'inverse modulo utilise l'_algorithme d'Euclide étendu_ <<22>>.

Vous remarquerez que ce script contient la fonction _powmod_ utilisée pour le RSA (nous n'en avons pas besoin pour les calculs à venir){nbsp}.
...
La fonction  _ec_mul_ pourrait ainsi être plus efficace, en appelant directement les fonctions ec_add_core et ec_dbl_core (dans la fonction ec_mul).
======================================
...
Au passage, on peut vérifier que le point G (donc de coordonénes g[0] pour G~x~ et g[1] pour G~y~ dans _secp256r1.b_) fait bien partie de la courbe{nbsp}:
...
* *[[[23]]]* Conception d'algorithmes et applications (LI325) COURS 10{nbsp}: http://www.di.ens.fr/~busic/cours/LI325/coursCAAC10.pdf
