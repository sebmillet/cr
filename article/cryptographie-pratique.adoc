...
****
La structure du certificat est décrite par une _grammaire_, d'après les règles de syntaxe *ASN.1* (_Abstrat{nbsp}Syntax{nbsp}Notation{nbsp}number{nbsp}1_){nbsp}<<1>>.
...
En ASN.1 le mot-clé `SEQUENCE` sans autre précision indique que la valeur est constituée d'une suite de valeurs elles-mêmes spécifiées en ASN.1. La valeur _Certificate_ contient donc, à la suite{nbsp}:
...
* Le _sujet_ du certificat a pour CN (_Common Name_) letsencrypt.org et c'est le dernier nom qui est affiché dans la hiérarchie des certificats (partie supérieure de la fenêtre).
...
Cela dit, le nom (au sens du standard x509) est constitué du DN (_Distinguished Name_), il s'agit de la _la totalité des éléments qui le composent_ (et non pas seulement du CN).
...
.Quelques précisions
****
* Pour qu'un certificat joue le rôle émetteur, il doit être une autorité de certification ou CA (_Certificate Authority_), il s'agit de l'une des nombreuses options que l'on peut trouver dans un certificat.
...
* Windows{nbsp}: les binaires sont accessibles ici{nbsp}<<7>>.
La version standard de _bc_ (GnuWin32) gère mal le clavier français. La référence <<7>> vous propose un exécutable _bc.exe_ qui n'a pas ce défaut.

[NOTE]
.Le choix de bc
======================================
* _bc_ est installé par défaut sur la plupart des distributions Linux, et facile et rapide à installer sous Windows (si ce n'est les problèmes de clavier français).
...
* *python*, langage de programmation en licence GPL, calcule par défaut sur des entiers de taille arbitrairement grande et convient donc aux calculs que nous allons faire. Beaucoup de tuto sur Internet utilisent _python_.
...
[caption=""]
====
Windows ne dispose pas par défaut d'outil pour passer du texte en majuscule. L'une des solutions est d'installer les utilitaires adéquats avec _GNUWin32_ ou bien _cygwin_.

L'éditeur de texte (natif) adapté pour ce type de transformation est *_notepad++_*, disponible à cette URL{nbsp}<<8>>.
...
A l'arrivée, *_val.b_* contient trois variables, _s_ (la signature du certificat), _n_ et _e_ (la clé publique).
On a ajouté la sauvegarde et la récupération d'_ibase_, c'est une bonne pratique.
...
Pour ceux qui utilisent _bc_ comme moi, il faut écrire la fonction.
...
[IMPORTANT]
Si vous exécutez _obase{nbsp}={nbsp}16_, vous obtiendrez le résultat voulu (_obase_ défini à _16 décimal_) seulement si _ibase_ vaut _10_.
Pour parer à toute éventualité, le plus simple est d'utiliser _2{nbsp}*{nbsp}8_ comme ici (ou bien _2{nbsp}^{nbsp}4_, ou quoi que ce soit qui n'utilise que des nombres à un seul chiffre).
...
* Sous Linux _openssl_ est disponible par défaut.
* Sous Windows il faut trouver un binaire _openssl_ à télécharger.
...
footnote:[BER signifie Basic Encoding Rules]
, CER
footnote:[CER signifie Canonical Encoding Rules]
ou DER
footnote:[DER signifie Distinguished Encoding Rules]
.
...
[**T**]ag::
Définit toutes les caractéristiques de la valeur, notamment son _type_, par exemple une date (`UTCTime`), une séquence (`SEQUENCE` ou `SEQUENCE{nbsp}OF`), un entier (`INTEGER`), et bien d'autres. Il indique également si l'encodage de la valeur est _primitive_ ou _contructed_.
Encodage _primitive_:::
La donnée ne contient pas de sous-structure
Encodage _constructed_:::
La donnée est elle-même une structure qui suit la typologie **T**-**L**-**V**, et ici, cette structure est elle-même définie en ASN.1{nbsp}footnote:[Certains types de données, notamment les _*string_ (`OCTET{nbsp}STRING`, `UTF8String`, etc.), peuvent être _primitive_ ou _constructed_, ce qui signifie que leur encodage peut suivre une structure hiérarchique "propre à l'encodage", non explicitée dans la définition de la valeur en ASN.1.].
...
+
Dans la définition d'un _Certificate_ ci-dessus, le type `SEQUENCE` est, par définition du type `SEQUENCE` en ASN.1, _constructed_. Cela signifie que la valeur `SEQUENCE` est elle-même une structure ASN.1, ce qu'on peut voir dans la liste des trois composants d'un certificat définie entre accolades.
...
Chaque fois que le tag indique une valeur _constructed_ (ici, il s'agit à deux reprises du type `SEQUENCE`, qui est obligatoirement _constructed_), on descend d'un niveau dans la structure hiérarchique, que l'on a fait ressortir ici avec l'option _-recursive "|  "_.
...
.Extraction de tbsCertificate{nbsp}: 1ère méthode (à la main)
...
Supprimer les deux premières lignes et un certain nombre de lignes à la fin pour que seul demeure le contenu de tbsCertificate.
Ci-dessous, le texte à conserver est le texte sélectionné (les lignes du milieu ont été supprimées pour condenser l'image).
...
.Extraction de tbsCertificate{nbsp}: 2ème méthode (à l'aide de _pkfile_)
...
Le résultat visible sur la figure 27 nous montre que le tbsCertificate correspond à la valeur `SEQUENCE` de 1518 octets (4 octets d'en-tête, 1514 octets de données). _pkfile_ donne à cette valeur les coordonnées 1.1.
...
Les captures d'écran montrent également le calcul sur le fichier _tbs2.der_ (créé avec _pkfile_), qui est ben identique à _tbs.der_, ce qui confirme au passage l'équivalence des deux méthodes pour extraire tbsCertificate.
...
. Le navigateur vérifie si les certificats rencontrés ont été révoqués à l'aide des Listes de Révocation des Certificats ou _CRL_ (_Certificate Revocation List_).
...
La notation _F~2^m^~_ est un raccourci. On a _F~2^m^~ = F~2~[x]/(p(x))_ où _p(x)_ est un polynôme irréductible de degré _m_ (source{nbsp}: <<15>>). Les "nombres" manipulés sont des polynômes sur _F~2~_ et les opérations sont faites modulo _p(x)_. L'équation de la courbe (source{nbsp}: <<16>>) est
+
_y^2^ + xy = x^3^ + ax^2^ + b_
+
Les opérations sur _F~2^m^~_ portent sur des polynômes et leur arithmétique est différente de l'arithmétique usuelle sur les nombres entiers. Par exemple quel que soit _A_ de _F~2^m^~_, on a _A{nbsp}+{nbsp}A{nbsp}={nbsp}0_ (soit _A{nbsp}={nbsp}-A_ pour tout _A_).
...
** _G_, le générateur d'un sous-groupe du groupe _E(F~2^m^~)_.
...
** _G_, le générateur d'un sous-groupe du groupe _E(F~p~)_.
** _n_ est l'ordre du groupe généré par _G_.
...
_Seed_ est un nombre arbitraire à partir duquel les autres paramètres de la courbe ont été déterminés, de manière vérifiable par n'importe qui, selon une spécification _X9.62_.
Cela garantit qu'ils n'ont pas été choisis en raison de propriétés cachées qui permettraient à leur inventeur de casser la protection facilement.
_Seed_ est à l'origine des paramètres de la courbe mais il n'en fait pas partie.
Le calcul des paramètres d'après _Seed_ utilise une fonction de hachage à sens unique, ce qui garantit que le choix ne s'est pas fait dans l'autre sens (détermination de paramètres ad hoc puis du _Seed_ correspondant).
...
. Choisir un entier _k_ inférieur à _n_
...
Le point _Q_ est la clé publique, l'entier _k_ est la clé privéefootnote:[Un peu plus loin la clé privée sera notée _d_.].
Ainsi contrairement au RSA où la clé privée et publique ont même nature (ce sont deux entiers), ici la clé publique est un point tandis que la clé privée est un entier.
...
[NOTE]
Le script présenté plus loin contient la fonction _powmod_ utilisée précédemment, pour le RSA, mais nous n'en avons pas besoin pour les calculs ECDSA.
...
======================================
_bc_ permet de déclarer une fonction _void_ pour qu'elle ne renvoie pas de valeur, exemple{nbsp}:

 define void myprint(x) { print "x=", x, "\n" }

La version Windows de _bc_ (GnuWin32) est ancienne et ne reconnaît pas le mot-clé _void_. Le script _crypto.b_ a été conçu pour être compatible avec cette version. Pour éviter qu'une expression (ici l'appel d'une fonction) ne produise un affichage parasite, chaque appel est précédé de ".=". Ainsi le résultat est affecté à la variable _last_ (qui vaut toujours la dernière valeur calculée). Cela ne modifie donc pas le cacul. C'est la méthode usuelle avec _bc_ pour ne pas afficher le résultat d'une expression.
======================================

=== Les courbes elliptiques dans le standard x509
...
Sur _F~p~_ on a _y~1~{nbsp}={nbsp}-y~2~_ (car non seulement _y_ apparaît sous la forme _y^2^_, mais en plus, il est isolé à gauche du signe _égal_) et comme nous sommes dans l'arithmétique modulaire on peut ré-écrire cette égalité _y~1~{nbsp}{plus}{nbsp}y~2~{nbsp}={nbsp}p_.
...
======================================
Pour retrouver le _y_ qui satisfasse l'équation _y^2^ = x^3^ + ax + b_, il faut calculer le terme à droite du signe égal puis en extraire la racine carrée (dans _Z/pZ_{nbsp}: il s'agit de la _racine modulaire_), et enfin choisir le bon _y_ d'après le bit de parité.

D'après <<23>>, il n'existe pas d'algorithme déterministe qui calcule la racine modulaire en temps polynomial. Cette même source donne un algorithme de type Monte-Carlo (qui fait donc intervenir le hasard) en _temps moyen_ polynomial, ce qui est suffisant dans la pratique.
======================================
...
.Création d'une paire de clés privée et publique sur prime256v1

Exécuter footnote:[Enregistrer une clé privée dans un fichier non chiffré est une mauvaise pratique. D'autre part le fichier doit être enregistré dans un endroit parfaitement sécurisé, ne jamais être transféré à travers un réseau, etc. Ici cela n'a pas d'importance puisque la clé est une clé de test.]
...
Par définition, un certificat auto-signé a même émetteur (_issuer_) et sujet (_subject_).
Sa clé sera celle du fichier _k.pem_.

Exécuter la commande{nbsp}:

 $ openssl req -new -x509 -key k.pem -subj "/CN=Sebastien Millet DEV CA/C=FR" -out cert.pem

Contrôler le résultat avec cette commande{nbsp}:

 $ openssl x509 -in cert.pem -noout -text

[caption="Fig. 36", title=" "]
image::img-openssl-autosigned-redim.png["Création certificat auto-signé"]

==== Vérification de la signature ECDSA du certificat

.Signature d'un message avec ECDSA

Les formules sont disponibles sur de nombreux sites, par exemple ici{nbsp}<<24>>.

Ce tutoriel (en anglais) est très bien fait et explique dans le détail de nombreux calculs{nbsp}<<25>>.

Jusqu'à présent _k_ désignait la clé privée. Ci-dessous _k_ est un entier choisi aléatoirement, alors que la clé privée est notée _d_.

Les notations sont celles introduites précédemment pour les paramètres de domaine ainsi que la clé publique.
Le message à signer est noté _m_ et son hash est noté _H(m)_.

. Choisir un entier k entre _0_ et _n_, différent de _0_ et de _n_

. Calculer image:img-sig-formula-1.png["ECDSA Formule 1"] Si _r_ est égal à _0_, recommencer à l'étape _1_

. Calculer image:img-sig-formula-2.png["ECDSA Formule 2"] Si _s_ est égal à zéro, recommencer à l'étape _1_

[NOTE]
La division par _k_ est une multiplication par _k^-1^{nbsp}mod{nbsp}n_.

[IMPORTANT]
_k_ doit être cryptographiquement aléatoire, et différent à chaque signature. Si une clé signe deux documents différents avec un _k_ unique, il est trivial de la calculer. Le lien <<25>> en donne la démonstration et décrit un cas pratique avec le programme _openssl_ modifié pour signer avec un _k_ invariable.

Le couple _(r,{nbsp}s)_ est la signature de _H(m)_.

.Vérification d'une signature ECDSA

. Calculer image:img-sig-formula-3.png["ECDSA Formule 3"] et image:img-sig-formula-4.png["ECDSA Formule 4"]

[start=2]
. Calculer image:img-sig-formula-5.png["ECDSA Formule 5"]

[NOTE]
Dans les formules de calcul de _u~1~_ et _u~2~_, on pourrait se passer du deuxième _mod{nbsp}n_, mais le calcul serait un peu plus long.

[start=3]
. Vérifier que _p{nbsp}={nbsp}r_

.Vérification de notre certificat auto-signé

L'encodage de _(r, s)_ est standardisé par la RFC 5280. Il s'agit de la définition suivante{nbsp}:
...
Si vous vous rappelez les chapitres précédents concernant le RSA, la signature (ici la valeur de _(r,{nbsp}s)_) est la troisième séquence du certificat. Avec _pkfile_ il s'agit du noeud de coordonnées _1.3.1_.

Nous allons récupérer les valeurs de _r_ et _s_ du certificat et les écrire dans le fichier _sign.b_.
Deux possibilités pour afficher _r_ et _s_{nbsp}:

. Avec _pkfile_ (la première commande permet de voir que nous avons besoin de décoder la valeur _1.3.1_){nbsp}:
+
 pkfile cert.pem -l 3
 pkfile cert.pem -N 1.3.1
+

[caption="Fig. 37", title=" "]
image::img-pkfile-cert-1-redim.png["pkfile 1"]
[caption="Fig. 38", title=" "]
image::img-pkfile-cert-2-redim.png["pkfile 2"]

. Avec _openssl_ (la première commande permet de voir que nous avons besoin de décoder la valeur située à l'offset _351_){nbsp}:
+
 openssl asn1parse -in cert.pem
 openssl asn1parse -in cert.pem -strparse 351
+
[caption="Fig. 39", title=" "]
image::img-openssl-strparse-redim.png["openssl straparse"]

Nous avons également besoin de _H(m)_, que nous noterons _hm_ dans notre script (dans certains tutoriels il est noté _e_). Comme vu avec le RSA il faut extraire la partie tbsCertificate de notre certificat et en calculer le hash.

Quel hash{nbsp}? Le certificat indique que l'algorithme est _ecdsa-with-SHA256_, donc _hm{nbsp}={nbsp}SHA256(tbsCertificate)_.

.Calcul de hm

. Extraire tbsCertificate dans _tbs.der_{nbsp}:
+
 pkfile cert.pem -n 1.1 -x -o tbs.der

. Calculer le hash SHA256 de _tbs.der_{nbsp}:

.Linux
[caption=""]
====
 $ sha256sum tbs.der
[caption="Fig. 40", title=" "]
image::img-pkfile-cert-4-redim.png["pkfile 4"]
====
...
====
 pkfile --sha256 tbs.der
[caption="Fig. 41", title=" "]
image::img-pkfile-cert-3-redim.png["pkfile 3"]
====

Après ces manipulations le fichier _sign.b_ contient _r_, _s_ et _hm_.
...
Ne reste plus qu'à faire les calculs{nbsp}:

[caption="Fig. 42", title=" "]
image::img-sig-verif-1-redim.png["Vérification signature  1"]

On voit que l'abscisse du résultat est bien égale à _r_ ce qui valide la signature.

=== Les Certificats Électroniques Visibles

==== Présentation

Le CEV ou _Certificat Électronique Visible_ est un certificat au format 2D-Doc, mis en place par l'ANTS (_Agence Nationale des Titres Sécurisés_).
Il est décrit à cette URL{nbsp}<<26>>.

Le lien <<26>> permet de télécharger le document _ANTS_2D-Doc_CABSpec_v2.0.1_erratum.pdf_, qui en donne tous les détails techniques ainsi que quelques exemples. Par la suite nous le désignerons simplement comme "le document 2D-Doc".

==== Vérification manuelle d'une signature 2D-Doc

Nous allons vérifier la signature du code donné en page 59 du document 2D-Doc{nbsp}:

[caption="Fig. 43", title=" "]
image::img-2ddoc-expg58-redim.png["Exemple certificat 2D-Doc page 58"]

Nous utiliserons le fichier _2Ddoc-values.b_ pour stocker les nombres correspondants.

. En page 58 du document 2D-Doc se trouve le certificat de l'autorité de certification de test (l'_émetteur_). Copier-coller le certificat dans un éditeur de texte et l'enregistrer dans le fichier _0001.pem_.

. Extraire de ce fichier les valeurs de la clé publique à l'aide d'_openssl_. Le document indique que la clé est de type _NIST{nbsp}P-256_, ce qui correspond à la courbe _prime256v1_ que nous avons étudiée précédemment et pour laquelle nous disposons déjà des paramètres (fichier _secp256r1.b_).
+
...
. Copier-coller la partie signée du document ("Données signées") dans le fichier _msg2ddoc-signed.bin_.

. Extraire les variables _r_ et _s_ de la signature ("Signature (binaire)").

[IMPORTANT]
Dans le document signé (enregistré dans _msg2ddoc-signed.bin_), les séquences _<GS>_ doivent être remplacées par le code ASCII _29_, et il faut savoir qu'il y a un espace (et un seul) entre _DES_ et _SPECIMENS_ (l'affichage n'est pas clair sur ce point).

Au final le fichier _2Ddoc-values.b_ contient ceci{nbsp}:

.2Ddoc-values.b
[source, bc]
----
include::2Ddoc-values.b[]
----

Comme précédemment, il ne nous reste plus qu'à faire les calculs.

[caption="Fig. 44", title=" "]
image::img-2ddoc-verif-manual-redim.png["Vérification 2D-Doc manuelle"]

_t[0]_ est l'abscisse de _u~1~G{nbsp}+{nbsp}u~2~Q_ et comme attendu il est ben égal à _r_, ce qui valide la signature.

Pour être rigoureux il aurait fallu comparer _r_ avec _t[0]{nbsp}%{nbsp}n_, comme ici _t[0]{nbsp}<{nbsp}n_ nous avons omis ce calcul.

==== Vérification d'une signature 2D-Doc avec _openssl_
...
. En hexadécimal, ajouter _02{nbsp}21{nbsp}00_ avant _r_, et _02{nbsp}20_ avant _s_. Ensuite ajouter _30{nbsp}45_ au tout début du fichier.
** Pourquoi ajouter un zéro devant _r_{nbsp}? L'entier _r_ commence par _CE_ (base 16). Le type `INTEGER` en ASN.1 est signé.
Tel quel, _r_ serait lu comme négatif du fait du bit de poids fort égal à 1.
_s_ commence par _3A_ (base 16) et n'a donc pas besoin d'être préfixé avec un octet nul.
+
Sans cet ajout d'un octet nul devant _r_, _openssl_ lit un entier négatif égal à _r{nbsp}-{nbsp}2^256^_{nbsp}:
_-3170DA8166986B0A01740C6A093800CB61984A4B39F7B1994015051EDD355451_ (base 16)
. Enregistrer le fichier (hexadécimal) comme _2ddoc-signature-wrapped.hex_.
+
.2ddoc-signature-wrapped.hex
----
include::2ddoc-signature-wrapped.hex[]
----

. Convertir le contenu hexadécimal en binaire et l'enregistrer comme _2ddoc-signature-wrapped.bin_. C'est cette signature qui sera examinée par _openssl_. Vous pouvez vérifier le résultat avec la commande
+
 $ openssl asn1parse -inform der -in 2ddoc-signature-wrapped.bin
+
[caption="Fig. 45", title=" "]
image::img-2ddoc-asn1parse-redim.png["2D-Doc parse asn1"]
+
. Le certificat (fichier _0001.pem_) ne peut pas être exploité tel quel par _openssl{nbsp}dgst_, il faut en extraire la clé publique. Exécuter la commande suivante{nbsp}:
+
 openssl x509 -in 0001.pem -noout -pubkey > 0001-pubkey.pem
+
Le fichier _0001-pubkey.pem_ contient la clé publique de "0001", et rien d'autre.

Il ne nous reste plus qu'à demander à _openssl_ de vérifier tout cela. Exécuter la commande{nbsp}:

 openssl dgst -sha256 -verify 0001-pubkey.pem -signature 2ddoc-signature-wrapped.bin msg2ddoc-signed.bin

Résultat{nbsp}:
[caption="Fig. 46", title=" "]
image::img-2ddoc-verif-with-openssl-redim.png["Vérification 2D-Doc avec openssl"]

_openssl_ nous dit `Verified OK`.

Voilà qui clôt ces manipulations autour de la cryptographie à courbes elliptiques dans les cadres x509 et 2D-Doc.

N'oubliez pas que la validation d'un certificat nécessite d'autres étapes que la seule vérification d'une signature ECDSA.
Si vous devez valider un certificat dans un environnement de production, seul un logiciel adapté pourra le faire de manière fiable.
...
+
[IMPORTANT]
Le fichier exécutable _bc.exe_ semble mal gérer le clavier français, à cause de _readline_. Cela oblige à jongler pour saisir des caractèrs tels que '^', '{' et '[', et il faut parfois passer en mode "clavier anglais" pour s'en sortir. Une alternative est de compiler _bc_ sans _readline_. L'auteur de ce document a compilé _bc_ sans _readline_ et l'a publié ici{nbsp}: http://seb-o-fourneaux.fr/?p=116
...
* *[[[24]]]* Elliptic curve digital signature algorithm{nbsp}: https://fr.wikipedia.org/wiki/Elliptic_curve_digital_signature_algorithm
* *[[[25]]]* ECC Tutorial (en anglais){nbsp}: http://www.johannes-bauer.com/compsci/ecc
* *[[[26]]]* Le 2D-Doc de l'ANTS{nbsp}: https://ants.gouv.fr/Les-solutions/2D-Doc
