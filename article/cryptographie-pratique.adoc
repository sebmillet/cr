...
* _ECDSA_{nbsp}: nous commencerons par travailler sur un certificat auto-signé que nous créerons pour l'occasion, afin de se familiariser avec les calculs qu'implique la cryptographie à courbes elliptiques. Pour finir nous récupérerons un certificat CEV (Certificat Électronique Visible) footnote:[Ce dernier cas sort du cadre x509.]
...
_p_ est un nombre premier. On a _F~p~ = Z/pZ_ (source{nbsp}: <<17>>). Les opérations sont faites modulo _p_. L'équation de la courbe est
+
_y^2^ (mod p) = x^3^ + ax + b (mod p)_
...
** _G_, le générateur du groupe noté _E(F~2^m^~)_ (_G_ est un point de la courbe).
** _n_ est l'ordre de la courbe.
...
** _G_, le générateur du groupe noté _E(F~p~)_ (_G_ est un point de la courbe).
** _n_ est l'ordre de la courbe.
...
Les calculs sont faits footnote:[Dans la signature ECDSA certains calculs sont faits modulo _n_, l'ordre du groupe associé à la courbe. Cela sera signalé. À défaut, tous les calculs sont faits modulo _p_.]
modulo _p_ et nous ne le préciserons plus par la suite.
...
. Calculer _Q = kG_ (voir ci-après la multiplication d'un point par un scalaire)
...
Les formules qui en découlent sont les suivantes, pour _R{nbsp}={nbsp}P{nbsp}+{nbsp}Q_ de coordonnées (_r~x~_, _r~y~_), (_p~x~_, _p~y~_) et (_q~x~_, _q~y~_) respectivement, _P_ étant non aligné verticalement avec _Q_ (_p~x~_ est différent de _q~x~_){nbsp}:
...
Un point de la courbe d'abscisse _x_ ne peut avoir que deux valeurs de _y_, notons les _y~1~_ et _y~2~_.

Ils sont opposés (_y~1~{nbsp}={nbsp}-y~2~_) et comme nous sommes dans l'arithmétique modulaire on peut ré-écrire cette égalité _y~1~{nbsp}{plus}{nbsp}y~2~{nbsp}={nbsp}p_.

_p_ étant impair, _y~1~_ et _y~2~_ n'ont pas même parité (l'un est pair, l'autre est impair).
La RFC 5480footnote:[En fait la RFC renvoie au document "SEC 1: Elliptic Curve Cryptography" pour les détails.]
permet de coder un point de deux manières, compressé ou non compressé.

. Le format non compressé consiste à enregistrer les deux coordonnées intégralement.

. Le format compressé consiste à enregistrer _x_ et la parité de _y_.
Pour "décompresser" un point compressé (calculer _y_ d'après _x_ et l'information sur la parité de _y_) il faut calculer la racine carrée modulaire de _x^3^{nbsp}{plus}ax{nbsp}{plus}{nbsp}b_.
L'algorithme qui le permet dans le cas général, et en un nombre borné d'étapes n'est pas trivial.

Le codage d'un point en x509 (c'est-à-dire, une clé publique) impose de commencer par un octet qui identifie la suite{nbsp}:

* Premier octet = 02{nbsp}: le point est compressé, et _y_ est pair. _x_ est codé juste après 02.
* Premier octet = 03{nbsp}: le point est compressé, et _y_ est impair. _x_ est codé juste après 03.
* Premier octet = 04{nbsp}: le point est non compressé. _x_ est codé juste après 04, et _y_ est codé juste après _x_.
...
Pour représenter un point de la courbe nous utilisons la convention _point[0] = abscisse du point_ et _point[1] = ordonnée du point_.
...
Au passage, on peut vérifier que le point G (de coordonénes g[0] pour G~x~ et g[1] pour G~y~ dans _secp256r1.b_) fait bien partie de la courbe{nbsp}:
...
* *[[[15]]]* Peter Seibt, Andreea Dragut Cours de cryptographie Chapitre V{nbsp}: http://www.dil.univ-mrs.fr/~jfp/master/m03/CompresChap5AES.pdf
...
* *[[[17]]]* Corps finis{nbsp}: http://iml.univ-mrs.fr/~rodier/Cours/RappelCorps%20finis.pdf
* *[[[18]]]* SEC 2: Recommended Elliptic Curve Domain Parameters{nbsp}: http://www.secg.org/SEC2-Ver-1.0.pdf
* *[[[19]]]* RECOMMENDED ELLIPTIC CURVES FOR FEDERAL GOVERNMENT USE{nbsp}: http://csrc.nist.gov/groups/ST/toolkit/documents/dss/NISTReCur.pdf
...
* *[[[98]]]* Paramètres de domain brainpool{nbsp}: http://www.ecc-brainpool.org/download/Domain-parameters.pdf
